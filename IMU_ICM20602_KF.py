...
if __name__ == '__main__':
    ...
    # for i in range(0, 100000):
        ...
        x_output[0:3] = x_ba_k * 57.3
        x_output[3:6] = np.array([data_truth[i + 1][8], data_truth[i + 1][9], data_truth[i + 1][10]])
        x_output[6:9] = x_sum * 57.3
        outputdata.append(x_output.copy())
        print(i)

        # xx.append((x_ba_k[0] * 57.3))

    # np.savetxt("outputData/output_ICM20602_KF_rpy.txt", outputdata, delimiter=" ")
    """[  roll_KF,pitch_KF, yaw_KF,roll_truth,pitch_truth,yaw_truth,roll_sum,pitch_sum, yaw_sum]"""
    plt.plot(yy, color='r')
    plt.show()
