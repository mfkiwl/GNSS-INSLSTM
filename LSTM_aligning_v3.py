...
if __name__ == '__main__':
    ...
    for i in range(0, len(data_IMU)):
        ...
        # 更新循环量
        p_k1 = p_k
        x_ba_k1 = data_truth[count_truth][8:11] * np.pi / 180.0
        # =========================================================================================================
        # 输出
        data_aligning[0] = data_IMU[i][0]  # time

        data_aligning[1:3] = z_k[0:2] * np.pi / 180.0  # 加速度得到的角

        data_aligning[3] = yaw_GNSS
        data_aligning[4:7] = x_ba_k *57.3
        data_aligning[7:10] = data_truth[count_truth][8:11]
        data_output.append(data_aligning.copy())
        """[ seconds of week,gx, gy, gz, vx,vy,vz,latitude, longitude,roll, Pitch, Yaw,]"""
        # print(data_aligning[0])
    np.savetxt("outputData/data1_300.txt", data_output, delimiter=" ")
