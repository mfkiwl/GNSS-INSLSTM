...
if __name__ == '__main__':
    ...
    step_time=600
    if data_from_txt:
        data = np.genfromtxt("outputData/data1_300.txt", dtype=float)  # 将文件中数据加载到data数组里
        """[ seconds of week,roll, Pitch, Yaw,latitude, longitude,roll, Pitch, Yaw,]"""
        data_input = []
        data_output = []
        # for i in range(10):
        for i in range(0,len(data) - step_time,10):
            data_input.append(np.array(data[i:i + step_time, [3,6]]))

            data_output.append(np.array(data[i + step_time][9]))
...
# model.add(Dense(8, activation='relu'))
model.add(Dense(1, activation='relu'))  # 输出层的单元数与形状匹配
# # # 编译模型
