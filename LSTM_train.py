...
if __name__ == '__main__':
    ...
    step_time=500
    if data_from_txt:
        data = np.genfromtxt("outputData/output_aligning_ICM20602.txt", dtype=float)  # 将文件中数据加载到data数组里
        """[ seconds of week,roll, Pitch, Yaw,latitude, longitude,roll, Pitch, Yaw,]"""
        data_input = []
        data_output = []
        # for i in range(10):
        for i in range(0,len(data) - step_time,10):
            data_input.append(np.array(data[i:i + step_time, 1:6]))
            data_output.append(np.array(data[i + step_time][6:9]))
...
model.add(LSTM(8, return_sequences=False, activation='relu', input_shape=(step_time, 5)))  # 使用None作为时间步长，使得模型可以处理任意长度的序列
# model.add(LSTM(16, return_sequences=False, activation='relu'))  # 不再需要返回序列，因为只关心最后的输出
# model.add(Dense(8, activation='relu'))
model.add(Dense(5, activation='relu'))
model.add(Dense(3, activation='relu'))  # 输出层的单元数与形状匹配

# # # 编译模型
model.compile(optimizer='adam', loss='mse', metrics=['mse', 'mae'])

model.load_weights('1w_2.keras')

# # # 打印模型概要
model.summary()
# # # 训练模型
model.fit(X_train, y_train, epochs=10, batch_size=8, validation_data=(X_test, y_test), callbacks=[tb_callback])
# model.fit(X_train, y_train, epochs=20, batch_size=32, validation_data=(X_test, y_test), callbacks=[tb_callback])

# # # 保存模型
model.save('1w_3.keras')
#
